...
class Range:
    def __init__(self, start, end, incr=Number(1)):
        self.start = start
        self.end = end
        self.incr = incr

    def __repr__(self):
        if self.incr == Number(1):
            return f"Range({repr(self.start)}, {repr(self.end)})"
        else:
            return f"Range({repr(self.start)}, {repr(self.end)}, {repr(self.incr)})"

    def __eq__(self, other):
        return (isinstance(other, Range) and self.start == other.start 
                    and self.end == other.end and self.incr == other.incr)


    def __iter__(self):
        # Assuming the environment is not needed for iteration
        # If needed, it should be provided when creating the Range instance
        start_value = self.start.execute({})
        end_value = self.end.execute({})
        incr = self.end.execute({})

        if end_value > start_value:
            return iter(range(start_value, end_value + 1, incr))
        else:
            return iter(range(start_value, end_value -1, -incr))

    def execute(self, env):
        start_value = self.start.execute(env)
        end_value = self.end.execute(env)
        incr = self.incr.execute(env)
        if end_value > start_value:
            return List(*[Number(i) for i in range(start_value, end_value + 1, incr)]).execute(env)
        else:
            #return [Number(i) for i in range(start_value, end_value - 1, -1)]
            return List(*[Number(i) for i in range(start_value, end_value - 1, -incr)]).execute(env)
...
class Cat:
    ...
    def __repr__(self):
        return f"cat({repr(self.lst)}, {repr(self.element)})"

    def __eq__(self, other):
        return isinstance(other, Cat) and self.lst == other.lst and self.element == other.element

    def execute(self, env):
        list_elements = self.lst.execute(env)
        element = self.element.execute(env)
        return list_elements + [element]




class Subset:
    def __init__(self, lst, start, end, n=Number(1)):
        self.lst = lst
        self.start = start
        self.end = end
        self.n = n
    ...
    def __repr__(self):
        if self.n == Number(1):
            return f"Subset({repr(self.lst)}, {repr(self.start)}, {repr(self.end)})"

        else:
            return f"Subset({repr(self.lst)}, {repr(self.start)}, {repr(self.end)}, {repr(self.n)})"

    def __eq__(self, other):
        return (isinstance(other, Subset) and self.lst == other.lst and 
                self.start == other.start and self.end == other.end and self.n == other.n)

    def execute(self, env):
        list_elements = self.lst.execute(env)
        start_value = self.start.execute(env)
        end_value = self.end.execute(env)
        n_value = self.n.execute(env)

        # Check if the bounds and increment are valid
        if not isinstance(start_value, int) or not isinstance(end_value, int) or not isinstance(n_value, int):

            return []
        if start_value < 0 or n_value <= 0 or start_value >= end_value:
            return []

        return [list_elements[i] for i in range(start_value, min(end_value, len(list_elements)), n_value)]
...
class Exists:
    ...
    def execute(self, env):
        ...
        L = self.list.execute(env)
        for element in L:
            T = Apply(self.function, Number(element)).execute(env)
            if T:
                return True
        return False
...
class Forall:
    ...
    def execute(self, env):
        ...
        L = self.list.execute(env)
        for element in L:
            if not Apply(self.function, Number(element)).execute(env):
                return False
        return True
